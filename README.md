...
The companion plugin uses a bundled MultiRent SVG icon in the WordPress left admin menu. The public site header shows a logo only when **MultiRent Setup > Website Setup > Page logo** has an image selected; when that field is empty, the header shows the property name without any logo.
