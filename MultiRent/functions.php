...
define( 'MULTIRENT_VERSION', '0.3.0' );
...
function multirent_global_font_choices() {
	return array(
		'theme-default' => array(
			'label' => esc_html__( 'Theme default', 'multirent' ),
			'css'   => '',
		),
		'playfair'      => array(
			'label' => esc_html__( 'Playfair Display', 'multirent' ),
			'css'   => '"Playfair Display", Georgia, serif',
		),
		'inter'         => array(
			'label' => esc_html__( 'Inter', 'multirent' ),
			'css'   => '"Inter", Arial, sans-serif',
		),
		'poppins'       => array(
			'label' => esc_html__( 'Poppins', 'multirent' ),
			'css'   => '"Poppins", Arial, sans-serif',
		),
		'montserrat'    => array(
			'label' => esc_html__( 'Montserrat', 'multirent' ),
			'css'   => '"Montserrat", Arial, sans-serif',
		),
		'lora'          => array(
			'label' => esc_html__( 'Lora', 'multirent' ),
			'css'   => '"Lora", Georgia, serif',
		),
		'merriweather'  => array(
			'label' => esc_html__( 'Merriweather', 'multirent' ),
			'css'   => '"Merriweather", Georgia, serif',
		),
		'raleway'       => array(
			'label' => esc_html__( 'Raleway', 'multirent' ),
			'css'   => '"Raleway", Arial, sans-serif',
		),
		'oswald'        => array(
			'label' => esc_html__( 'Oswald', 'multirent' ),
			'css'   => '"Oswald", Arial, sans-serif',
		),
		'nunito-sans'   => array(
			'label' => esc_html__( 'Nunito Sans', 'multirent' ),
			'css'   => '"Nunito Sans", Arial, sans-serif',
		),
		'source-sans-3' => array(
			'label' => esc_html__( 'Source Sans 3', 'multirent' ),
	...
	if ( $items ) {
		...
		<ul id="primary-menu" class="menu">
			<?php foreach ( $items as $item ) : ?>
				...
				<li class="<?php echo $is_current ? 'current-menu-item current_page_item' : ''; ?>"><a href="<?php echo esc_url( $item['url'] ); ?>"<?php echo $is_current ? ' aria-current="page"' : ''; ?>><?php echo esc_html( $item['label'] ); ?></a></li>
			<?php endforeach; ?>
		</ul>
		<?php
		return;
...
function multirent_customize_register( $wp_customize ) {
	...
	$settings = array(
		'property_name'     => array(
			'label'   => esc_html__( 'Property name', 'multirent' ),
			'default' => esc_html__( 'Your Rental Property', 'multirent' ),
			'type'    => 'text',
		),
		'property_tagline'  => array(
			'label'   => esc_html__( 'Property tagline', 'multirent' ),
			'default' => '',
			'type'    => 'text',
		),
		'global_font'       => array(
			'label'   => esc_html__( 'Global site font', 'multirent' ),
			'default' => 'theme-default',
			'type'    => 'select',
		),
		'hero_eyebrow'      => array(
			'label'   => esc_html__( 'Hero eyebrow', 'multirent' ),
			'default' => esc_html__( 'Multi-unit rental website', 'multirent' ),
			'type'    => 'text',
		),
		'hero_title'        => array(
			'label'   => esc_html__( 'Hero title', 'multirent' ),
			'default' => esc_html__( 'Flexible stays for every guest', 'multirent' ),
			'type'    => 'text',
		),
		'hero_text'         => array(
			'label'   => esc_html__( 'Hero text', 'multirent' ),
			'default' => esc_html__( 'Showcase apartments, rooms, villas, or holiday homes with clear details and easy inquiry paths.', 'multirent' ),
			'type'    => 'textarea',
		),
		'hero_title_size'   => array(
			'label'   => esc_html__( 'Hero title size in cm', 'multirent' ),
			'default' => '',
			'type'    => 'number',
		),
		'hero_button_text'  => array(
			'label'   => esc_html__( 'Hero button text', 'multirent' ),
			'default' => esc_html__( 'View rentals', 'multirent' ),
			'type'    => 'text',
		),
		'hero_button_url'   => array(
			'label'   => esc_html__( 'Hero button URL', 'multirent' ),
			'default' => '#rentals',
			'type'    => 'url',
		),
		'intro_eyebrow'     => array(
			'label'   => esc_html__( 'Intro eyebrow', 'multirent' ),
			'default' => esc_html__( 'About the property', 'multirent' ),
			'type'    => 'text',
		),
		'intro_title'       => array(
			'label'   => esc_html__( 'Intro title', 'multirent' ),
			'default' => esc_html__( 'Manage every unit from WordPress', 'multirent' ),
			'type'    => 'text',
		),
		'intro_text'        => array(
			'label'   => esc_html__( 'Intro text', 'multirent' ),
			'default' => esc_html__( 'Add rental units, amenities, photos, capacity, booking links, and guest-facing details from the WordPress dashboard.', 'multirent' ),
			'type'    => 'textarea',
		),
		'stats_lines'       => array(
			'label'   => esc_html__( 'Homepage stats, one per line: value | label', 'multirent' ),
			'default' => "4 | Rental units\n100 m | Example distance\n24/7 | Self-managed content",
			'type'    => 'textarea',
		),
		'reviews_shortcode' => array(
			'label'   => esc_html__( 'Reviews shortcode', 'multirent' ),
			'default' => '',
			'type'    => 'text',
		),
		'color_scheme'      => array(
			'label'   => esc_html__( 'Color scheme', 'multirent' ),
			'default' => 'coastal',
			'type'    => 'select',
		),
		'use_custom_colors' => array(
			'label'   => esc_html__( 'Use custom colors', 'multirent' ),
			'default' => '0',
			'type'    => 'checkbox',
		),
	);
	...
	foreach ( array( 'primary', 'dark', 'surface', 'accent' ) as $color_key ) {
		...
		$wp_customize->add_control(
			new WP_Customize_Color_Control(
				...
				'color_' . $color_key,
				array(
					'label'   => sprintf( /* translators: %s: color role name such as primary, dark, surface, or accent. */ esc_html__( 'Custom %s color', 'multirent' ), $color_key ),
					'section' => 'multirent_home',
				)
			)
		);
	...
	$wp_customize->add_control(
		new WP_Customize_Media_Control(
			...
			'hero_image',
			array(
				'label'     => esc_html__( 'Hero image', 'multirent' ),
				'section'   => 'multirent_home',
				'mime_type' => 'image',
			)
		)
	...
	$wp_customize->add_control(
		new WP_Customize_Media_Control(
			...
			'page_logo',
			array(
				'label'     => esc_html__( 'Page logo', 'multirent' ),
				'section'   => 'multirent_home',
				'mime_type' => 'image',
			)
		)
